...
ON *:TEXT:.anitv*:#: {
	var %api = http://anitv.foolz.us/

	if ($2) {
		var %url = %api $+ json.php?controller=search&query= $+ $json.enccomponent($2-) $+ &_timestamp= $+ $ctime
		var %idx = results
	}
	else {
		var %url = %api $+ json.php?controller=schedule&total=5&nowplaying=false&_timestamp= $+ $ctime
		var %idx = programs
	}
	...
	var %row = 0
	while (%row < %count) {
		msg $chan 7 $+ $json(%url,%index,%i,title)  episode 7 $chr(35) $+ $json(%url,%index,%i,episode)  airs on7 $json(%url,%index,%i,station) at7 $asctime($ctime($json(%url,%index,%i,airtime)),ddd HH:nn:ss) JST.  $chr(91) $+  $+ $replace($duration($calc($json(%url,%index,%i,unixtime) - $ctime)),secs,s,mins,m,hrs,h,days,d) $+  $+ $chr(93) 0- 12 $+ http://anidb.net/a $+ $json(%url,%index,%i,anidb)
		inc %row
	}
}
